...
يجب أن تفعل:
1. استخدم get_chatbot_submenu({parentCode: "02", lang: "a"})
2. اعرض القائمة الفرعية بالشكل التالي:
...
### 5. معالجة الخدمات المباشرة (ISMENU = 0):
```
المستخدم يختار: "أجنحة دسمان" أو "3"

يجب أن تفعل:
1. اعرض ACTION2TAKE مباشرة:
...
2. اعرض القائمة بالشكل التالي:

[بناءً على البيانات من API]
1. المواعيد (إذا كان ISMENU = 1)
2. الغرف (إذا كان ISMENU = 1)  
3. أجنحة دسمان (إذا كان ISMENU = 0)
4. الصيدلية (إذا كان ISMENU = 0)
5. مواعيد المراكز (إذا كان ISMENU = 0)
6. خدمة المختبر المنزلي (إذا كان ISMENU = 0)
7. خدمات أخرى (إذا كان ISMENU = 1)
8. إيقاف الإشعارات (إذا كان ISMENU = 0)
